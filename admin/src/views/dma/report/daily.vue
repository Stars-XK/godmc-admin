            ...
            >
              <el-table-column prop="name" label="分区名称" min-width="160" fixed="left">
                <template #default="scope">
                  <span class="zone-name-cell" :style="{ paddingLeft: (scope.row._level || 0) * 0 + 'px' }">
...
function handleExpandChange(row, expandedRows) {
  // Element Plus 的 expand-change 事件只传当前行和已展开行列表
}
...
function renderChart(dataList) {
  ...
  const dates = hasData ? dataList.map(d => d.date) : buildDateLabels(trendDays.value)
...
function renderEmptyChart() {
  ...
  const dates = buildDateLabels(trendDays.value)
